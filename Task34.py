...
# слов, то они разделяются дефисами. Фразы отделяются друг от друга пробелами. Стихотворение Винни-Пух вбивает в программу с клавиатуры.
...
words = input('Что сказал Винни-Пух ->  ').lower().split()
glas = 'ауоыиэяюёе'
ritm = True
sumglas0 = sum(map(lambda c: c in glas, words[0]))
for i in range(1, len(words)):
    if sumglas0 != sum(map(lambda c: c in glas, words[i])):
        ritm = False
        break
if ritm:
    print('Парам пам-пам')
